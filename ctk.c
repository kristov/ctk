...
static uint8_t realize_widget_menu_item(ctk_widget_t* widget) {
...
static uint8_t realize_widget_menu(ctk_widget_t* widget) {
    uint16_t max_width = 0;
    int length = strlen(widget->label);
    for (uint16_t i = 0; i < widget->nr_children; i++) {
        ctk_realize_widget(&widget->children[i]);
...
static uint8_t realize_widget_hbox(ctk_widget_t* widget) {
    uint16_t max_width = 0;
    for (uint16_t i = 0; i < widget->nr_children; i++) {
...
static uint8_t realize_widget_vbox(ctk_widget_t* widget) {
    uint16_t max_width = 0;
    uint16_t y = 0;
    for (uint16_t i = 0; i < widget->nr_children; i++) {
        ctk_realize_widget(&widget->children[i]);
...
uint8_t ctk_realize_widget(ctk_widget_t* widget) {
    ...
    switch (widget->type) {
        case CTK_WIDGET_MENU:
            return realize_widget_menu(widget);
            break;
        case CTK_WIDGET_MENU_ITEM:
            return realize_widget_menu_item(widget);
            break;
        case CTK_WIDGET_MENU_BAR:
            return realize_widget_hbox(widget);
            break;
        default:
            return realize_widget_vbox(widget);
            break;
    }
    return 0;
}
